...
public class EnumConstant extends CallableOperation {
	...
	/**
	 * Parses the description of an enum constant value in a string as returned
	 * by
	...
	public static void parse(String desc, TypedOperationManager manager) throws OperationParseException {
		...
		if (colonIdx < 0) {
			String msg = "An enum constant description must be of the form \"" + "<type>:<value>"
					+ " but description is \"" + desc + "\".";
	...
	/**
	 * valueOf searches the enum constant list of a class for a constant with
	 * the given name. Note: cannot make this work using valueOf method of Enum
	 * due to typing.
	 *
	 * @param type
	 ...
	 * @return reference to actual constant value, or null if none exists in
	 *         type.
	 */
