...
public final class NonreceiverTerm extends CallableOperation {
	 ...
	 * {@inheritDoc}
	 *
	 * @return {@link NormalExecution} object enclosing value of this
	 *         non-receiver term.
	 */
	@Override
	...
	/**
	 * {@inheritDoc} For NonreceiverTerm, simply adds a code representation of
	 * the value to the string builder. Note: this does not explicitly box
	 * primitive values.
	 *
	...
	/**
	 * Returns a NonreceiverTerm holding the zero value for the specified class
	 * c. In the case of characters there is no natural zero, so the value 'a'
	 * is used.
	 *
	 * @param type
	...
	static NonreceiverTerm createNullOrZeroTerm(ConcreteType type) {
		...
		if (type.equals(ConcreteTypes.CHAR_TYPE))
			return new NonreceiverTerm(type, 'a'); // TODO This is not null or
													// zero...
		if (type.equals(ConcreteTypes.BYTE_TYPE))
			return new NonreceiverTerm(type, (byte) 0);
		if (type.equals(ConcreteTypes.SHORT_TYPE))
		...
		if (type.equals(ConcreteTypes.BOOLEAN_TYPE))
			return new NonreceiverTerm(type, false);
		return new NonreceiverTerm(type, null);
	}

	/**
	 * {@inheritDoc} Returns a string representing this primitive declaration.
	...
	public static void parse(String s, TypedOperationManager manager) throws OperationParseException {
		...
		if (colonIdx == -1) {
			String msg = "A primitive value declaration description must be of the form " + "<type>:<value>"
					+ " but the description \"" + s + "\" does not have this form.";
		...
		if (typeString.matches(".*\\s+.*")) {
			String msg = "Error when parsing type/value pair " + s
					+ ". A primitive value declaration description must be of the form " + "<type>:<value>"
			...
			if (valString.equals("null")) {
				...
				if (!PrimitiveTypes.stringLengthOK((String) value)) {
					throw new OperationParseException(
							"Error when parsing String; length is greater than " + GenInputsAbstract.string_maxlen);
				}
			}
		} else {
			if (valString.equals("null")) {
