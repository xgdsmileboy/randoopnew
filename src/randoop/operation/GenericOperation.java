...
/**
 * {@code GenericOperation} is an abstract implementation of {@link Operation}
 * that represents an operation whose definition includes a type variable.
 * <p>
 * An operation is represented as a {@code GenericOperation} if
 * <ul>
 * <li>it is defined in a {@link GenericType} in which case it may have concrete
 * or generic type, or</li>
 * <li>it is a generic method or constructor of a concrete type.</li>
 * </ul>
 * This variability means that at least one of the types associated with a
 * {@code GenericOperation} must be generic, but the others may be concrete. So,
 * all types are represented by {@code GeneralType} objects.
 * <p>
 * Note that the <i>declaring</i> and <i>output</i> types of a creation
 * operation are both defined as the type created.
 */
public class GenericOperation extends TypedOperation<CallableOperation> {
	...
	/**
	 * Create a generic operation with the given declaring type, input types and
	 * output type.
	 *
	 * @param declaringType
	 *            the type in which the operation is declared
	 * @param inputTypes
	 *            the input types for the operation
	 * @param outputType
	 *            the output type for the operation
	 */
	public GenericOperation(CallableOperation operation, GeneralType declaringType, GeneralTypeTuple inputTypes,
	...
	public boolean equals(Object obj) {
		...
		return super.equals(op) && declaringType.equals(op.declaringType) && inputTypes.equals(op.inputTypes)
	...
	/**
	 * Returns the tuple of input types for this operation. If a method call or
	 * field access, the first input corresponds to the receiver, which must be
	 * an object of the declaring class.
	 *
	...
	/**
	 * Creates a {@link ConcreteOperation} from this generic operation by using
	 * the given {@link Substitution} on type variables.
	 *
	 * @param substitution
	 *            the type substitution
	 * @return the concrete operation with type variables replaced by
	 *         substitution
	 */
