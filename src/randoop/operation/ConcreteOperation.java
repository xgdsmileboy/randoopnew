...
/**
 * {@code ConcreteOperation} is an abstract implementation of {@link Operation}
 * that represents the operations that can occur within a test. A concrete
 * operation is either defined in a non-generic context, or is instantiated from
 * a {@link GenericOperation}.
 */
public class ConcreteOperation extends TypedOperation<CallableOperation> {
	...
	/**
	 * Creates a {@code ConcreteOperation} with the given input and output
	 * types.
	 *
	 * @param inputTypes
	 *            the tuple of concrete input types
	 * @param outputType
	 *            the concrete output type
	 */
	public ConcreteOperation(CallableOperation operation, ConcreteType declaringType, ConcreteTypeTuple inputTypes,
	...
	public ExecutionOutcome execute(Object[] input, PrintStream out) {
		assert input.length == inputTypes.size() : "operation execute expected " + inputTypes.size() + ", but got "
				+ input.length;
	...
	/**
	 * Produces a Java source code representation of this statement and append
	...
	public boolean equals(Object obj) {
		...
		return getOperation().equals(op.getOperation()) && declaringType.equals(op.declaringType)
				&& inputTypes.equals(op.inputTypes) && outputType.equals(op.outputType);
	}
	...
	public static ConcreteOperation createPrimitiveInitialization(ConcreteType type, Object value) {
		assert PrimitiveTypes.isBoxedOrPrimitiveOrStringType(type.getRuntimeClass()) : "must be nonreceiver type, got "
				+ type.getName();
