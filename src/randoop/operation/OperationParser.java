...
public class OperationParser {
	...
	/**
	 * Parses a string representing a StatementKind. The string is expected to
	 * be of the form:
	 *
	 * <pre>
	 *   ID : DESCRIPTION
	 * </pre>
	 *
	 * Where ID is a string identifying the type of StatementKind, and
	 * DESCRIPTION represents more specifics of the StatementKind. For example,
	 * the following String represents the constructor for HashMap:
	 *
	 * <pre>
	...
	public static void parse(String str, TypedOperationManager manager) throws OperationParseException {
		...
		validIds.addAll(Arrays.asList(NonreceiverTerm.ID, MethodCall.ID, ConstructorCall.ID, ArrayCreation.ID,
				EnumConstant.ID, FieldGet.ID, FieldSet.ID));
		...
		if (id.equals(NonreceiverTerm.ID)) {
			...
			String msg = "A statement description must be of the form " + "<id> <description>" + " with <id> in "
					+ validIds.toString() + " but the statement \"" + str + "\" does not have a valid <id>.";
			throw new OperationParseException(msg);
		}
	...
	public static String getId(ConcreteOperation op) {
		...
		if (op.getOperation() instanceof NonreceiverTerm)
			return NonreceiverTerm.ID;
		if (op.getOperation() instanceof MethodCall)
			return MethodCall.ID;
		if (op.getOperation() instanceof ConstructorCall)
			return ConstructorCall.ID;
		if (op.getOperation() instanceof ArrayCreation)
			return ArrayCreation.ID;
		if (op.getOperation() instanceof EnumConstant)
			return EnumConstant.ID;
		if (op.getOperation() instanceof FieldGet)
			return FieldGet.ID;
