...
public class Help extends CommandHandler {
	...
	public Help() {
		super("help", "Displays a help message for a given command.", "help", "",
				"Displays a help message for a given command.", null,
	...
	public boolean handle(String[] args_raw) throws RandoopTextuiException {
		...
		if (args.length == 0) {
			...
			for (CommandHandler h : Main.handlers) {
				out.println(
						Util.hangingParagraph(h.fcommand + " -- " + h.fpitch, Globals.COLWIDTH, Globals.INDENTWIDTH));
			}
			out.println();
			...
			if (args.length != 1) {
				throw new RandoopTextuiException("The `help' command must be followed by"
						+ " exactly one argument: the command that you" + " want help on.");
			}
