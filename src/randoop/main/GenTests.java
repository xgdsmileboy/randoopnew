...
public class GenTests extends GenInputsAbstract {
	...
	private static Options options = new Options(GenTests.class, GenInputsAbstract.class, ReflectionExecutor.class,
	...
	public boolean handle(String[] args) throws RandoopTextuiException {
		...
		Set<String> coveredClassnames = GenInputsAbstract.getStringSetFromFile(include_if_class_exercised,
				"Unable to read coverage class names");
		...
		Set<String> methodSignatures = GenInputsAbstract.getStringSetFromFile(methodlist,
				"Error while reading method list file");

		OperationModel operationModel = null;
		try {
			operationModel = OperationModel.createModel(visibility, reflectionPredicate, classnames, coveredClassnames,
					methodSignatures, classNameErrorHandler, GenInputsAbstract.literals_file);
		...
		/*
		 * Initialize components: - Add default seeds for primitive types - Add
		 * any values for TestValue annotated static fields in operationModel
		 */
...
//	    components.addAll(SeedSequences.objectsToSeeds(new ArrayList(list)));		
		...
		Set<String> observerSignatures = GenInputsAbstract.getStringSetFromFile(GenInputsAbstract.observers,
				"Unable to read observer file", "//.*", null);
		...
		explorer = new ForwardGenerator(model, observers, timelimit * 1000, inputlimit, outputlimit, componentMgr,
				listenerMgr);
		...
		Predicate<ExecutableSequence> isOutputTest = createTestOutputPredicate(excludeSet,
				operationModel.getExercisedClasses(), GenInputsAbstract.include_if_classname_appears);
	...
	/**
	 * Handles the occurrence of a {@code SequenceExceptionError} that indicates
	 * a flaky test has been found. Prints information to help user identify
	 * source of flakiness, including exception, statement that threw the
	 * exception, the full sequence where exception was thrown, and the input
	 * subsequence.
	 *
	 * @param explorer
	 *            the test generator
	...
	private void handleFlakySequenceException(AbstractGenerator explorer, SequenceExceptionError e) {
		...
		if (Log.isLoggingOn()) {
			...
			if (!callSet.isEmpty()) {
				...
				System.out.printf("Unable to find a previous occurrence of subsequence%n" + "%s%n"
						+ "where exception was thrown%n" + "Please submit an issue%n", subsequence);
			}
		}
	}
	...
	public Predicate<ExecutableSequence> createTestOutputPredicate(Set<Sequence> excludeSet,
			Set<Class<?>> coveredClasses, Pattern includePattern) {
	 ...
	 * for each sequence it is applied to. Validity and contract checks are
	 * always needed to determine which sequences have invalid or error
	 * behaviors, even if only regression tests are desired. So, this generator
	 * will always be built. If in addition regression tests are to be
	 * generated, then the regression checks generator is added.
	 *
	 * @param visibility
	 ...
	 *         arguments.
	 */
	public TestCheckGenerator createTestCheckGenerator(VisibilityPredicate visibility, Set<ObjectContract> contracts,
			MultiMap<ConcreteType, ConcreteOperation> observerMap, Set<ConcreteOperation> excludeAsObservers) {
		...
		if (!GenInputsAbstract.no_regression_tests) {
			...
			regressionVisitor = new RegressionCaptureVisitor(expectation, observerMap, excludeAsObservers,
					includeAssertions);
	 ...
	 **/
	private static List<File> writeJUnitTests(String output_dir, List<ExecutableSequence> seqList,
			List<String> additionalJUnitClasses, String junitClassname) {

		List<File> files = new ArrayList<>();

		if (seqList.size() > 0) {
			List<List<ExecutableSequence>> seqPartition = CollectionsExt
					.<ExecutableSequence> chunkUp(new ArrayList<>(seqList), testsperfile);
