...
public abstract class AbstractGenerator {
	...
	/**
	 * Sequence limit for generation. If generation reaches the specified
	 * sequence limit, the generator stops generating sequences.
	 */
	 ...
	 * declarations, etc.) used to generate sequences. In other words,
	 * statements specifies the universe of operations from which sequences are
	 * generated.
	 */
	 ...
	 *            the maximum number of sequences to generate. Must be
	 *            non-negative.
	 * @param maxOutSequences
	 *            the maximum number of sequences to output. Must be
	 *            non-negative.
	 ...
	 * @param stopper
	 *            Optional, additional stopping criterion for the generator. Can
	 *            be null.
	 * @param listenerManager
	 *            Manager that stores and calls any listeners to use during
	 *            generation. Can be null.
	 */
	public AbstractGenerator(List<ConcreteOperation> operations, long timeMillis, int maxGeneratedSequences,
			int maxOutSequences, ComponentManager componentManager, IStopper stopper,
	...
	/**
	 * Registers a visitor with this object for use while executing each
	 * generated sequence.
	 *
	 ...
	 * <li>the elapsed generation time is greater than or equal to the max time
	 * in milliseconds,
	...
	protected boolean stop() {
		return (listenerMgr != null && listenerMgr.stopGeneration()) || (timer.getTimeElapsedMillis() >= maxTimeMillis)
				|| (numOutputSequences() >= maxOutputSequences) || (numGeneratedSequences() >= maxGeneratedSequences)
	...
	public void explore() {
		...
		if (!GenInputsAbstract.noprogressdisplay) {
			...
			System.out.println();
			System.out.println("Average method execution time (normal termination):      "
					+ String.format("%.3g", ReflectionExecutor.normalExecAvgMillis()));
			System.out.println("Average method execution time (exceptional termination): "
	 ...
	 * subsequences, which can be retrieved using
	 * {@link #getSubsumedSequences()}
	 *
