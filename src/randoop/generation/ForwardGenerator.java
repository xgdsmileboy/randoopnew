...
public class ForwardGenerator extends AbstractGenerator {
	...
	public ForwardGenerator(List<ConcreteOperation> operations, Set<ConcreteOperation> observers, long timeMillis,
			int maxGenSequences, int maxOutSequences, ComponentManager componentManager,
			RandoopListenerManager listenerManager) {
		this(operations, observers, timeMillis, maxGenSequences, maxOutSequences, componentManager, null,
				listenerManager);
	}

	public ForwardGenerator(List<ConcreteOperation> operations, Set<ConcreteOperation> observers, long timeMillis,
			int maxGenSequences, int maxOutSequences, ComponentManager componentManager, IStopper stopper,
			RandoopListenerManager listenerManager) {
	...
	/**
	 * Determines what indices in the given sequence are active. An active index
	 * i means that the i-th method call creates an interesting/useful value
	 * that can be used as an input to a larger sequence; inactive indices are
	 * never used as inputs. The effect of setting active/inactive indices is
	 * that the SequenceCollection to which the given sequences is added only
	 * considers the active indices when deciding whether the sequence creates
	...
	private void processSequence(ExecutableSequence seq) {
		...
		if (!seq.isNormalExecution()) {
			if (Log.isLoggingOn()) {
				...
				Log.logLine("Statement with non-normal execution: "
		...
		for (int i = 0; i < seq.sequence.size(); i++) {
			...
			// and if the value is new, add a sequence corresponding to that
			// value.
			...
			if (PrimitiveTypes.isBoxedOrPrimitiveOrStringType(objectClass)) {
				...
				if (!looksLikeObjToString && !tooLongString && runtimePrimitivesSeen.add(runtimeValue)) {
					// Have not seen this value before; add it to the component
					// set.
	...
	private ExecutableSequence createNewUniqueSequence() {
		...
		if (newSequence.size() > GenInputsAbstract.maxsize) {
			if (Log.isLoggingOn())
				Log.logLine("Sequence discarded because size " + newSequence.size() + " exceeds maximum allowed size "
	 ...
	 * this object as a prefix, repeating the operation the given number of
	 * times. Used during generation.
	 *
	 * @param operation
		...
		for (int i = 0; i < times; i++) {
			...
			for (Variable v : retval.getInputs(retval.size() - 1)) {
				if (v.getType().equals(ConcreteTypes.INT_TYPE)) {
					...
					retval = retval
							.extend(ConcreteOperation.createPrimitiveInitialization(ConcreteTypes.INT_TYPE, randint));
		...
		if (GenInputsAbstract.debug_checks) {
			...
			if (this.allSequences.contains(newSequence)) {
				...
				if (this.allsequencesAsCode.contains(code)) {
					...
					assert co.equals(newSequence); // XXX this was a floating
													// call to equals
					b.append("new component:").append(Globals.lineSep).append("").append(newSequence.toString())
							.append("").append(Globals.lineSep).append("as code:").append(Globals.lineSep).append("")
							.append(code).append(Globals.lineSep);
					b.append("existing component:").append(Globals.lineSep).append("")
							.append(this.allsequencesAsList.get(index).toString()).append("").append(Globals.lineSep)
							.append("as code:").append(Globals.lineSep).append("")
	...
	// object and returned. If an appropriate collection of sequences and
	// indices
	// was not found (e.g. because there are no sequences in the
	// componentManager
	...
	private InputsAndSuccessFlag selectInputs(ConcreteOperation operation) {
		...
		// We denote this goal sequence as "S". We don't create S explicitly,
		// but
		// define it as the concatenation of the following list of sequences.
		// In other words, S = sequences[0] + ... +
		// sequences[sequences.size()-1].
		// (This representation choice is for efficiency: it is cheaper to
		// perform
		...
		//
		// and the singleton list [0] that represents variable var1. The
		// variable
		...
		// Their purpose is purely to improve efficiency. For a given loop
		// iteration
		// i, "types" contains the types of all variables in S, and
		// "typesToVars"
		...
		for (int i = 0; i < inputTypes.size(); i++) {
			...
			if (GenInputsAbstract.alias_ratio != 0 && Randomness.weighedCoinFlip(GenInputsAbstract.alias_ratio)) {
				...
				// candidateVars will store the indices that can serve as input
				// to the
				// i-th input in st.
				List<SimpleList<Integer>> candidateVars = new ArrayList<>();

				// For each type T in S compatible with inputTypes[i], add all
				// the
				// indices in S of type T.
				for (ConcreteType match : types.getMatches(inputType)) {
					// Sanity check: the domain of typesToVars contains all the
					// types in
					// variable types.
					assert typesToVars.keySet().contains(match);
					candidateVars.add(new ArrayListSimpleList<>(new ArrayList<>(typesToVars.getValues(match))));
				}

				// If any type-compatible variables found, pick one at random as
				// the
				// i-th input to st.
			...
			// If we got here, it means we will not attempt to use a value
			// already
			// defined in S,
			// so we will have to augment S with new statements that yield a
			// value of
			// type inputTypes[i].
			// We will do this by assembling a list of candidate sequences
			// n(stored in
			// the list declared
			// immediately below) that create one or more values of the
			// appropriate
			// type,
			// randomly selecting a single sequence from this list, and
			// appending it
			// to S.
			SimpleList<Sequence> l;

			// We use one of two ways to gather candidate sequences, but the
			// second
			// case below
			...
			if (inputType.isArray()) {
				...
				// 1. If T=inputTypes[i] is an array type, ask the component
				// manager for
				// all sequences
				// of type T (list l1), but also try to directly build some
				// sequences
				...
				// 2. COMMON CASE: ask the component manager for all sequences
				// that
			...
			if (l.size() == 0) {
				if (isReceiver || GenInputsAbstract.forbid_null) {
					...
					// possible values to reuse, so we don't update typesToVars
					// or types.
					continue;
				}
			}
			...
			// However, the user may have requested that we use null values as
			// inputs
			...
			// At this point, we have a list of candidate sequences and need to
			// select
			// a
			...
			// We are not done yet: we have chosen a sequence that yields a
			// value of
			// the required
			// type inputTypes[i], but there may be more than one such value.
			// Our last
			// random
			...
			// Fail, if we were unlucky and selected a null or primitive value
			// as the
			...
			if (GenInputsAbstract.alias_ratio != 0) {
				...
				for (int j = 0; j < chosenSeq.size(); j++) {
					...
					if (stk.isPrimitiveInitialization())
						continue; // Prim decl not an interesting candidate for
									// multiple
					// uses.
