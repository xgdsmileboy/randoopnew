...
/**
 * Common superclass representing types in Randoop. Intended as glue for dealing
 * with generating objects from {@code java.lang.reflect.Type} objects,
 * especially instances of {@code java.lang.reflect.ParameterizedType} that can
 * either represent generic classes or a parameterized type.
 * <p>
 * This class is package private because it should not be necessary to use the
 * class directly within Randoop. The intent is that Randoop will only use the
 * subclasses {@link ConcreteType} and {@link GenericType}. Instances of
 * {@code GenericType} are only be used during harvesting of classes under test
 * using reflection, then these are instantiated to instances of
 * {@link ConcreteType} that are then used to generate the tests.
 *
...
public abstract class GeneralType {
	...
	/**
	 * Returns the runtime {@code Class} object for this type. For use when
	...
	/**
	 * Indicates whether the given {@code Class} object corresponds to the
	 * runtime class of this type.
	 *
	 * @param c
	 *            the class to check
	...
	/**
	 * Indicate whether this type is generic. If not, then type is concrete.
	 *
	 ...
	 * transitive closure of definition of the <i>direct supertype</i> relation
	 * in <a href=
	 * "https://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.10.2">
	 * section 4.10.2 of JLS for JavaSE 8</a>. <i>Only</i> checks reference
	 * types.
	 * 
	 ...
	 * {@code java.lang.reflect.Type} reference. If the object is a
	 * {@code Class} instance then returns the corresponding
	 * {@code ConcreteType}. If the type is actually a
	 ...
	 * {@code GenericClassType}. If the type is a
	 * {@code java.lang.reflect.GenericArrayType}, then the corresponding
	 * {@code GenericArrayType} is returned.
	 * <p>
	 ...
	 * getActualTypeArguments()} method to maintain the guarantees needed for
	 * {@link ParameterizedType#isSubtypeOf(ConcreteType)}.
	 *
	 * @param type
	 *            the type to interpret
	 * @return a {@code randoop.types.Type} object corresponding to the given
	 *         type
	 * @throws IllegalArgumentException
	 *             if the rawtype is not a Class instance
	 */
	...
	/**
	 * Performs the conversion of {@code java.lang.reflectGenericArrayType} to a
	 * {@code GenericArrayType} or a {@code ConcreteArrayType} depending on
	 * whether the component type is generic or concrete.
	 *
	 * @param type
	 *            the {@link java.lang.reflect.GenericArrayType} reference
	...
	private static GeneralType forParameterizedType(java.lang.reflect.ParameterizedType t) throws RandoopTypeException {
		...
		// Could be *actual* "actual type arguments", type variables, or
		// wildcards
		...
		if (typeParameters.size() == actualArguments.length) { // is generic
			...
			// obtained through the java.lang.reflect.ParameterizedType as
			// above.
			// Otherwise, the variables mapped by the substitutions used in
			// checking
			// subtyping will not be the correct objects, and the subtype test
			// will
			// fail.
			return new GenericClassType((Class<?>) rawType, typeParameters);
		} else if (typeParameters.isEmpty()) { // is parameterized type
			// When building parameterized type, first create generic class from
			// the
			// rawtype, and then instantiate with the arguments collected from
			// the
	...
	/**
	 * Returns a {@code GeneralType} object for the given type name. Uses
	 * reflection to find the correspond type.
	 *
	 * @param typeName
	 *            the name of a type
	 * @return the type object for the type with the name, null if none is found
	 * @throws ClassNotFoundException
	 *             if name is not a recognized type
	 */
