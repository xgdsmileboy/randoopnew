...
/**
 * Represents a parameterized type as a generic class instantiated with concrete
 * type arguments.
 * <p>
 ...
 * generic class. Conversion to this type from this and other
 * {@link java.lang.reflect.Type} interfaces is handled by
...
public class ParameterizedType extends ConcreteType {
	 ...
	 * @param substitution
	 *            the substitution for type variables
	 * @param argumentList
	...
	/**
	 * {@inheritDoc} Test if the given object is equal to this parameterized
	 * type. Two parameterized types are equal if they have the same raw type
	...
	public boolean equals(Object obj) {
		...
		for (int i = 0; i < typeParameters.size(); i++) {
			if (!(substitution.get(typeParameters.get(i).getParameter())
	...
	/**
	 * {@inheritDoc} Tests for assignability to a parameterized type: can the
	 * given concrete type be assigned to this parameterized type.
	 * <p>
	 ...
	 * type. The method {@link Class#isAssignableFrom(Class)} checks widening
	 * reference conversion, but with parameterized types it is also necessary
	 * to check type arguments. Since a widening reference conversion (JLS,
	 * section 5.1.5) exists from type S to type T if S is a subtype of T, this
	 * method calls {@link ParameterizedType#isSubtypeOf(ConcreteType)} to test
	...
	/**
	 * {@inheritDoc} Handles specific cases of supertypes of a parameterized
	 * type <code>C&lt;T<sub>1</sub>,...,T<sub>n</sub>&gt;</code> instantiating
	 * the generic type <code>C&lt;F<sub>1</sub>,...,F<sub>n</sub>&gt;</code> by
	 * substitution
	 * <code>&#952; =[F<sub>1</sub>/T<sub>1</sub>,...,F<sub>n</sub>]</code> for
	 * which direct supertypes are:
	 * <ol>
	 ...
	 * <li><code>C&lt;S<sub>1</sub>,...,S<sub>n</sub>&gt;</code> where S
	 * <sub>i</sub> <i>contains</i> T<sub>i</sub> (JLS section 4.5.1).
	 *
	 * <li>The rawtype <code>C</code>.
	 * <li><code>Object</code> if generic form is interface with no interfaces
	 * as supertypes.
	 * </ol>
	 * Note that the full definition of the <i>contains</i> relation in the
	 * second clause involves wildcards, which are not currently implemented,
	 * and so the relation reduces to equality on the parameter bounds. (Tested
	 * using {@link GenericClassType#equals(Object)}.)
	 */
	@Override
	public boolean isSubtypeOf(ConcreteType type) throws RandoopTypeException {
		...
		// if would-be supertype not parameterized, check supertype of generic
		// form
		...
		// variables of this.instantiatedType, which are shared by supertype
		// objects
		// created by Class.getGenericInterfaces() and
		// Class.getGenericSuperClass().
		// This is how GenericClassType.getMatchingSupertype(GenericClassType)
		// works.
		// If we get GenericClassType supertype via other means, the type
		// variables
		// will be distinct and the substitution will return null values even if
		// the
	...
	public String getName() {
		return getRuntimeClass().getCanonicalName() + "<" + UtilMDE.join(this.getTypeArguments(), ",") + ">";
	}
	 ...
	 *            the generic class type
	 * @return true if this type is an instantiation of the generic class, false
	 *         otherwise
	 */
	...
	public ConcreteType getSuperclass() throws RandoopTypeException {
		...
		assert (superclass instanceof GenericClassType) || (superclass instanceof ConcreteType) : "unexpected type: "
				+ superclass;
