 ...
 * <p>
 * In Randoop, meant to be instantiated to a {@code ConcreteType} rather than be
...
public abstract class GenericType extends GeneralType {
	 ...
	 * reference. The type must either represent a generic array, or a generic
	 * class.
	 * 
	 * @see randoop.types.GeneralType#forType(Type)
	 *
	 * @param type
	 *            the object from which the generic type is to be built
	 * @return a {@code GenericType} object constructed from the given type
	 * @throws IllegalArgumentException
	 *             if the type is neither a generic array or class
	 */
	...
	/**
	 * Builds a generic type from a {@code Class} object. (In Randoop, only
	 * expect to see {@code Class} objects for class under test, and so this
	 * method does not support constructing arrays from them.)
	 *
	 * @param c
	 *            the class
	 * @return a generic type object representing the class
	 * @throws IllegalArgumentException
	 *             if the class is not a generic type, or {@code Class} object
	 *             represents an array
	 */
