...
/**
 * Represents a type bound on a type variable occurring as a type parameter in a
 * class, interface, method or constructor. (See JLS section 4.4) In Java, a
 * type bound is either a type variable, a class type, an interface type, or an
 * intersection type of class and interface bounds. This class represents a
 * bound as concretely as possible based on the values returned by
...
public abstract class TypeBound {
	...
	/**
	 * Creates a {@code TypeBound} object from the given array of bounds. If
	 * there is more than one type, the returned bound is an intersection type.
	 *
	 * @param bounds
	 *            the types representing a type parameter bound
	 ...
	 * {@code java.lang.reflect.Type}. Tests for types that are represented by
	 * {@code Class} objects, or {@code java.lang.reflect.ParameterizedType}
	 * objects.
	 *
	 * @param type
	 *            the type for type bound
	 * @return a type bound that ensures the given type is satisfied as an upper
	 *         bound
	 * @throws IllegalArgumentException
	 *             if a parameterized type is given but the rawtype is not a
	 *             Class object
	 */
	...
	/**
	 * Returns the runtime class for this type bound. Depending on implementing
	 * class, this may be {@code Object} or something closer to the bound. The
	 * returned value should not be used to test satisfiability of the bound.
	 *
