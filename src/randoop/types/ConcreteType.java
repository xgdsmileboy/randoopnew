...
public abstract class ConcreteType extends GeneralType {
	 ...
	 * variable of this type: <code>
	 * Variable<sub>this</sub> = Expression<sub>sourcetype</sub>.
	 * </code> In other words, this is a legal assignment. Based on the
	 * definition of <i>assignment context</i> in <a href=
	 * "https://docs.oracle.com/javase/specs/jls/se8/html/jls-5.html#jls-5.2">
	 * section 5.2 of the JDK 8 Java Language Specification</a> a value of one
	 * type is assignable to a variable of another type if the first type can be
	 * converted to the second by
	 * <ul>
	 ...
	 * </ul>
	 * And, if after all those conversions, the type is a raw type, an unchecked
	 * conversion may occur.
	 * <p>
	 * When implementing this method, be aware that the method
	 * {@link Class#isAssignableFrom(Class)} checks identity and reference
	 * conversions, and does a comparison of raw types for parameterized types.
	 * The method {@link PrimitiveTypes#isAssignable(Class, Class)} checks
	 * identity and primitive widening for primitive types.
	 *
	 * @param sourceType
	 *            the type to test for assignability
	 * @return true if this type can be assigned from the source type, and false
	 *         otherwise
	 */
	...
	/**
	 * Indicate whether this type is a parameterized type. (A parameterized type
	 * is a generic class that has been instantiated with concrete type
	 * arguments such as <code>List&lt;String&gt;</code>.)
	 *
	...
	/**
	 * Indicate whether this type is a rawtype of a generic class. The rawtype
	 * is the runtime type of the class with type parameters erased.
	 *
	 * @return true if this type is a rawtype of a generic class, false
	 *         otherwise
	 */
	...
	/**
	 * Indicates whether the given object represents a value that is assignable
	 * to this type. If the reference is null, then returns true only if this
	 * type is not primitive.
	 *
	 * @param e
	 *            the element to check
	 * @return true if the type of {@code e} is assignable to this type, false
	 *         otherwise
	 */
	 ...
	 * <p>
	 * For a primitive, enum, or non-generic class, the arguments should be
	 * empty. If {@code typeClass} is a generic class, then a
	 * {@link ParameterizedType} is created as long as the number of actual type
	 * arguments agrees with the number of type parameters. Otherwise, if
	 * {@code typeClass} is a generic class given without arguments, then the
	 * type is a rawtype, and a {@link ConcreteSimpleType} is created.
	 *
	 * @param typeClass
	 *            the {@code Class} object
	 * @param arguments
	 *            the actual type arguments to create a parameterized type
	 ...
	 *
	 * @param elementType
	 *            the element type for the array type
	 * @return a type object representing the array type with the given element
	 *         type
	 */
