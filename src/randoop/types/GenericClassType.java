 ...
 * formal parameter or return type. Related to concrete
 * {@link ParameterizedType} by instantiating with a {@link Substitution}.
 */
public class GenericClassType extends GenericType {
	...
	/**
	 * Create a {@code GenericClassType} object for the given rawtype. Uses the
	 * {@link java.lang.reflect.GenericDeclaration} methods to access the type
	 * parameters for the generic class.
	 * <p>
	 * This is the standard constructor to use when only a {@code Class} object
	 * is available. It should not be used when creating instances for
	 * supertypes where type variables are needed to be the same, such as during
	 ...
	 *
	 * @param rawType
	 *            the raw type for the generic class
	...
	/**
	 * Create a {@code GenericClassType} for the given rawtype with the
	 * parameters, and parameter type bounds.
	 * <p>
	 * This constructor is intended to mainly be used by
	 * {@link randoop.types.GeneralType#forType(Type)} where the full set of
	 * arguments is collected before creating the type object.
	 *
	 * @param rawType
	 *            the rawtype for the generic class
	 * @param parameters
	...
	/**
	 * {@inheritDoc} Checks that the rawtypes are the same. This is sufficient
	 * since the type parameters and their bounds can be reconstructed from the
	 * Class object.
	 *
	 * @return true if two generic classes have the same rawtype, false
	 *         otherwise
	 */
	@Override
	 ...
	 * @throws IllegalArgumentException
	 *             if a type argument does not match the corresponding type
	 *             parameter bounds
	 */
	@Override
	...
	/**
	 * Instantiates this generic class using the substitution to replace the
	 * type parameters.
	 *
	 * @param substitution
	 *            the type substitution
	 * @return a {@link ParameterizedType} instantiating this generic class by
	...
	/**
	 * Returns a direct supertype of this type that either matches the given
	 * type, or has a rawtype assignable to (and so could be subtype of) the
	 * given type. Returns null if no such supertype is found. Construction
	 * guarantees that substitution on this generic class type will work on
	 * returned generic supertype as required by
	 * {@link ParameterizedType#isSubtypeOf(ConcreteType)}.
	 *
	 * @param type
	 *            the potential supertype
	 * @return a supertype of this type that matches the given type or has an
	 *         assignable rawtype; or null otherwise
	 * @throws IllegalArgumentException
	 *             if type is null
	 */
	GenericClassType getMatchingSupertype(GenericClassType type) throws RandoopTypeException {
		...
		if (otherRawType.isInterface()) {
			...
			for (Type t : interfaces) {
				...
				if (generalType.isGeneric() && type.equals(generalType)) { // found
																			// the
																			// type
