...
public class OperationExtractor implements ClassVisitor {
	...
	public void visit(Constructor<?> c) {
		assert c.getDeclaringClass().equals(classType.getRuntimeClass()) : "classType " + classType
				+ " and declaring class " + c.getDeclaringClass().getName() + " should be same";
		if (!predicate.test(c)) {
			return;
		}
	 ...
	 *
	 * @param method
	 *            a {@link Method} object to be represented as an
	 *            {@link Operation}.
	 */
	@Override
	public void visit(Method method) {
		assert method.getDeclaringClass().isAssignableFrom(classType.getRuntimeClass()) : "classType " + classType
				+ " should be assignable to declaring class " + method.getDeclaringClass().getName();
	 ...
	 *
	 * @param field
	 *            a {@link Field} object to be represented as an
	 *            {@link Operation}.
	 */
	@Override
	public void visit(Field field) {
		assert field.getDeclaringClass().isAssignableFrom(classType.getRuntimeClass()) : "classType " + classType
				+ " should be assignable from " + field.getDeclaringClass().getName();
		...
		manager.createTypedOperation(new FieldGet(accessibleField), classType, new GenericTypeTuple(getInputTypeList),
				fieldType);
		if (!accessibleField.isFinal()) {
			setInputTypeList.add(fieldType);
			manager.createTypedOperation(new FieldSet(accessibleField), classType,
					new GenericTypeTuple(setInputTypeList), ConcreteTypes.VOID_TYPE);
		}
	 ...
	 *
	 * @param e
	 *            an {@link Enum} object to be represented as an
	 *            {@link Operation}.
	 */
	@Override
