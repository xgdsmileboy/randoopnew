...
/**
 * {@code OperationModel} represents the information context from which tests
 * are generated. The model includes:
 * <ul>
 ...
 * </ul>
 * <p>
 * This class manages all information about generic classes internally, and
 * instantiates any type variables in operations before returning them.
 */
public class OperationModel extends ModelCollections {
	...
	/**
	 * Factory method to construct an operation model for a particular set of
	 * classes
	 *
	 * @param visibility
	 ...
	 *            members are used
	 * @param classnames
	 *            the names of classes under test
	 * @param exercisedClassnames
	 *            the names of classes to be tested by exercised heuristic
	 * @param literalsFileList
	 *            the list of literals file names
	 * @return the operation model for the parameters
	 */
	public static OperationModel createModel(VisibilityPredicate visibility, ReflectionPredicate reflectionPredicate,
			Set<String> classnames, Set<String> exercisedClassnames, Set<String> methodSignatures,
			ClassNameErrorHandler errorHandler, List<String> literalsFileList)
					throws OperationParseException, NoSuchMethodException, RandoopTypeException {
		...
		for (String classname : exercisedClassnames) {
			if (!classnames.contains(classname)) {
				...
				if (!visibility.isVisible(c)) {
					System.out
							.println("Ignorning non-visible " + c + " specified as include-if-class-exercised target");
				} else if (c.isInterface()) {
	...
	private List<Substitution> getSubstitutions(Set<ConcreteType> inputTypes, GenericClassType classType)
	...
	/**
	 * Selects all input types that potentially satisfies the upper bound. If
	 * the bound is concrete, then returned list exactly satisfies the bound. If
	 * the bound is generic, then the types are convertible a la
	 * Class.isAssignableFrom. Otherwise, the input types are returned as a
	 * list.
	 *
	 * @param bound
	 *            the (upper) bound to test
	 * @param inputTypes
	 *            the set of input types to test
	...
	/**
	 * Creates and adds the Object class default constructor call to the
	 * concrete operations.
	 */
	 ...
	 * specified by the user. Includes literals at different levels indicated by
	 * {@link ClassLiteralsMode}.
	 *
	 * @param compMgr
	...
	public void addClassLiterals(ComponentManager compMgr, List<String> literalsFile, ClassLiteralsMode literalsLevel) {
		...
		// Add a (1-element) sequence corresponding to each literal to the
		// component
		// manager.
	 ...
	 *
	 * @param observerSignatures
	 *            the set of method signatures
	 * @return the map to observer methods from their declaring class type
	 * @throws OperationParseException
	 *             if a method signature cannot be parsed
	 */
	public MultiMap<ConcreteType, ConcreteOperation> getObservers(Set<String> observerSignatures)
			throws OperationParseException {
	...
	/**
	 * Returns the set of identified {@code Class<?>} objects for the exercised
	 * class heuristic.
	 *
	...
	/**
	 * Returns the set of types for concrete (non-generic) classes in this
	 * model. Includes all instantiated generic classes.
	 *
	 ...
	 *
	 * @return true if the model has class types, and false if the class type
	 *         set is empty
	 */
	...
	/**
	 * {@code ObserverCollections} is a {@link ModelCollections} implementation
	 * that stores observer operations in a {@link MultiMap} provided to the
	 * constructor.
	 */
	private class ObserverCollections extends ModelCollections {
		...
		/**
		 * Creates an observer collection that stores observers in the given
		 * map.
		 *
		 * @param observerMap
		 *            the map to which this object adds observers
		 */
		...
		/**
		 * {@inheritDoc} Adds an observer operation for the given declaring type
		 * to the observer map.
		 */
		@Override
		public void addConcreteOperation(ConcreteType declaringType, ConcreteOperation operation) {
