...
/**
 * {@code ClassLiteralExtractor} is a {@link ClassVisitor} that extracts
 * literals from the bytecode of each class visited, adding a sequence for each
 * to a map associating a sequence with a type.
 */
	...
	public void visitBefore(Class<?> c) {
		...
		for (Class<?> constantClass : constantMap.keySet()) {
			...
			for (NonreceiverTerm term : constantMap.getValues(constantClass)) {
				Sequence seq = new Sequence().extend(ConcreteOperation.createNonreceiverInitialization(term),
