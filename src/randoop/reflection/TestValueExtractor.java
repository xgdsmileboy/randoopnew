...
/**
 * {@code TestValueExtractor} is a {@link ClassVisitor} that inspects the fields
 * passed to it to see if they are annotated with {@link randoop.TestValue}, are
 * static, and have type that is primitive, String, or an array of primitive or
 * String.
 */
public class TestValueExtractor implements ClassVisitor {
	...
	/**
	 * {@inheritDoc} If the field is static and is annotated with
	 * {@code randoop.TestValue}, adds the value of the field to the sequence
	 * set constructed by this visitor. Requires that the field type be
	...
	public void visit(Field f) {
		if (f.getAnnotation(TestValue.class) != null) {
			if (!Modifier.isStatic(f.getModifiers())) {
				String msg = "RANDOOP ANNOTATION ERROR: Expected @TestValue-annotated field " + f.getName()
						+ " in class " + f.getDeclaringClass() + " to be declared static but it was not.";
	...
	/**
	 * Returns the value stored in the given (static) field with primitive,
	 * String or array of primitive or String type.
	 *
	 * @param f
	 *            the field
	...
	private List<Object> getValue(Field f) {
		...
		if (PrimitiveTypes.isPrimitiveOrStringType(fieldType)
				|| (fieldType.isArray() && PrimitiveTypes.isPrimitiveOrStringType(fieldType.getComponentType()))) {
	...
	private static void printDetectedAnnotatedFieldMsg(Field f) {
		String msg = "ANNOTATION: Detected @TestValue-annotated field " + f.getType().getCanonicalName() + " \""
				+ f.getName() + "\" in class " + f.getDeclaringClass().getCanonicalName()
