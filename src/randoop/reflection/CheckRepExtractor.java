...
/**
 * {@code CheckRepExtractor} is a {@link ClassVisitor} that inspects the methods
 * passed to it to see if they are annotated with {@link randoop.CheckRep}, are
 * non-static, and have either a {@code boolean} or {@code void} return type.
 */
public class CheckRepExtractor implements ClassVisitor {
	...
	/**
	 * {@inheritDoc} If the method is annotated with {@link randoop.CheckRep},
	 * non-static, and with boolean or void return type, then a
	 * {@link randoop.contract.CheckRepContract} for the method is added to the
	 * contracts set.
	 *
	 * @param m
	 *            the method
	...
	public void visit(Method m) {
		if (m.getAnnotation(CheckRep.class) != null) {
			if (Modifier.isStatic(m.getModifiers())) {
				String msg = "RANDOOP ANNOTATION ERROR: Expected @CheckRep-annotated method " + m.getName()
	...
	private static void printDetectedAnnotatedCheckRepMethod(Method m) {
		String msg = "ANNOTATION: Detected @CheckRep-annotated method \"" + m.toString()
				+ "\". Will use it to check rep invariant of class " + m.getDeclaringClass().getCanonicalName()
				+ " during generation.";
