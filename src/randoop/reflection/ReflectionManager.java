 ...
 * </ul>
 * <p>
 * Note that visitors may have their own predicates, but need not check
 * visibility.
 */
public class ReflectionManager {
	 ...
	 *
	 * @param predicate
	 *            the predicate to indicate whether classes and class members
	 *            should be visited.
	 */
	 ...
	 *
	 * @param visitor
	 *            the {@link ClassVisitor} object to add
	 */
	 ...
	 * given class and its members that satisfy the given predicate. Excludes
	 * fields that are hidden by inheritance that are otherwise still accessible
	 * by reflection. Each visitor is applied to each member at most once.
	 *
	 * @param c
	 *            the {@link Class} object to be visited.
	 */
	public void apply(Class<?> c) {
		if (predicate.isVisible(c)) {
			...
			if (c.isEnum()) { // treat enum classes differently
				...
				for (Method m : c.getDeclaredMethods()) { // for all methods
															// declared by c
				...
				// Inner enums
				for (Class<?> ic : c.getDeclaredClasses()) { // look for inner
																// enums
				...
				// Fields
				// The set of fields declared in class c is needed to ensure we
				// don't
				// collect inherited fields that are hidden by local declaration
				Set<String> declaredNames = new TreeSet<>();
				for (Field f : c.getDeclaredFields()) { // for fields declared
														// by c
				...
				for (Field f : c.getFields()) { // for all public fields of c
					// keep a field that satisfies filter, and is not inherited
					// and hidden by
					// local declaration
	 ...
	 * method is included if it satisfies the predicate, and either is declared
	 * in the enum, or in the anonymous class of some constant. Note that
	 * methods will either belong to the enum itself, or to an anonymous class
	 * attached to a constant. Ordinarily, the type of the constant is the enum,
	 * but when there is an anonymous class for constant e, e.getClass() returns
	 * the anonymous class. This is used to check for method overrides (that
	 * could include Object methods) within the constant.
	 * <p>
	 * Heuristically exclude methods <code>values</code> and
	 * <code>valueOf</code> since their definition is implicit, and we aren't
	 * testing Java enum implementation.
	 *
	 * @param c
	 *            the enum class object from which constants and methods are
	 *            extracted
	 */
	private void applyToEnum(Class<?> c) {
		...
		for (Object obj : c.getEnumConstants()) {
			Enum<?> e = (Enum<?>) obj;
			applyTo(e);
			if (!e.getClass().equals(c)) { // does constant have an anonymous
											// class?
				for (Method m : e.getClass().getDeclaredMethods()) {
					overrideMethods.add(m.getName()); // collect any potential
														// overrides
				}
			}
		}
	...
	/**
	 * Determines whether a method, its parameter types, and its return type are
	 * all visible.
	 *
	 * @param m
	 *            the method to check for visibility
	 * @return true if the method, each parameter type, and the return type are
	 *         all visible; and false otherwise
	 */
	...
	/**
	 * Determines whether a concsturctor and each of its parameter types are
	 * visible.
	 *
	 * @param c
	 *            the constructor
	 * @return true if the constructor and each parameter type are visible;
	 *         false, otherwise
	 */
