...
public class ExecutableSequence {
	 ...
	 * during execution of this sequence. Invariant: sequence.size() ==
	 * executionResults.size(). Transient because it can contain arbitrary
	 * objects that may not be serializable.
	 */
	 ...
	 *
	 * @param sequence
	 *            the underlying sequence for this executable sequence
	 */
	 ...
	 *
	 * @param sequence
	 *            the base sequence of this object
	 * @param exec
	 *            the execution for this sequences
	 * @param checks
	 *            over the sequence
	 */
	 ...
	 * {@link randoop.test.ExceptionCheck}, that check's pre-statement code is
	...
	public String toCodeString() {
		...
		for (int i = 0; i < sequence.size(); i++) {
			...
			// of the sequence, because, otherwise, primitive values will be
			// used as
	 ...
	 * executing each statement via the method <code>getResult(i)</code>.
	 *
	 * <ul>
	 ...
	 * statement calls the given visitor's <code>visitBefore</code> method.
	 * After executing each statement, calls the visitor's
	 * <code>visitAfter</code> method.
	...
	public void execute(ExecutionVisitor visitor, TestCheckGenerator gen, boolean ignoreException) {
		...
		for (int i = 0; i < this.sequence.size(); i++) {
			...
			if ((statementResult instanceof ExceptionalExecution) && i < sequence.size() - 1) {
				if (ignoreException) {
					// this preserves previous behavior, which was simply to
					// return if
					// exception occurred
					break;
				} else {
	...
	private Object[] getRuntimeInputs(Sequence s, List<ExecutionOutcome> outcome, int i, List<Variable> inputs) {
	...
	/**
	 * Returns the values for the given variables in the {@link Execution}
	 * object. The variables are {@link Variable} objects in the
	 * {@link Sequence} of this {@link ExecutableSequence} object.
	 *
	 * @param vars
	...
	private static Object[] getRuntimeValuesForVars(List<Variable> vars, List<ExecutionOutcome> execution) {
		...
		for (int j = 0; j < runtimeObjects.length; j++) {
			...
			assert execution.get(creatingStatementIdx) instanceof NormalExecution : execution.get(creatingStatementIdx)
					.getClass();
	...
	private static void executeStatement(Sequence s, List<ExecutionOutcome> outcome, int index,
			Object[] inputVariables) {
	 ...
	 *
	 * @param i
	 *            the statement index to test for normal execution
	 ...
	 * @return The index in the sequence at which an exception of the given
	 *         class (or a class compatible with it) was thrown. If no such
	 *         exception, returns -1.
	 */
	 ...
	 * @param exceptionClass
	 *            the exception class
	 * @return True if an exception of the given class (or a class compatible
	 *         with it) has been thrown during this sequence's execution.
	 */
	 ...
	 *
	 * @return true if this sequence has non-executed statements, false
	 *         otherwise
	 */
	...
	/**
	 * Returns the index i for a non-executed statement, or -1 if there is no
	 * such index. Note that a statement is considered executed even if it
	 * throws an exception.
	 */
	 ...
	 *
	 * @return true if the test checks have been set and are for invalid
	...
	/**
	 * Adds a covered class to the most recent execution results of this
	 * sequence.
	 *
	 * @param c
