...
public final class ReflectionExecutor {
	 ...
	 * due to execution of code under test that results in an infinite loop.
	 * With this option, Randoop executes each test in a separate thread and
	 * kills tests that take too long to finish. Tests killed in this manner are
	 * not reported to the user. The downside is a BIG (order-of-magnitude)
	 * decrease in generation speed. The tests are not run in parallel, merely
	 * in isolation.
	 */
	 ...
	 * associated test, are stopped forcefully. Only meaningful if --usethreads
	 * is also specified.
	 */
	 ...
	 *
	 * @param code
	 *            the {@link ReflectionCode} to be executed
	 * @param out
	...
	private static void printExceptionDetails(Throwable e, PrintStream out) {
		...
		try {
			...
			try {
				...
				// Note that field Throwable.suppressedExceptions only exists in
				// JDK 7.
