...
public class ClassFileConstants {
	...
	// Byte, int, short, and char values are all stored in the .class file as
	// int.
	static byte bb = 23;
	 ...
	 * @throws IOException
	 *             if an error occurs in writing the constants
	 */
	 ...
	 *            the name of the type
	 * @param result
	 *            the set of constants to which constans are added
	...
	public static ConstantSet getConstants(String classname, ConstantSet result) {
		...
		for (Constant c : constant_pool.getConstantPool()) {
			...
			if (c == null || c instanceof ConstantClass || c instanceof ConstantFieldref
					|| c instanceof ConstantInterfaceMethodref || c instanceof ConstantMethodref
		...
		for (Method m : jc.getMethods()) {
			...
			if (il != null) {
				for (Instruction inst : il.getInstructions()) {
					...
					// These instructions compare the integer on the top of the
					// stack
					...
					case Const.IFLE: {
						...
						// Instanceof pushes either 0 or 1 on the stack
						// depending on
						// whether
						// the object on top of stack is of the specified type.
						// If were interested in class literals, this would be
						// interesting
					...
					case Const.DUP: {
						...
						// Duplicates the item on the top of the stack and
						// inserts it 2
					...
					case Const.DUP_X1: {
						...
						// Duplicates either the top 2 category 1 values or a
						// single
						// category 2 value and inserts it 2 or 3 values down on
						// the
						// stack.
					case Const.DUP2_X1: {
						break;
						...
						// Duplicate either one category 2 value or two category
						// 1 values.
					case Const.DUP2: {
						break;
					}

						// Dup the category 1 value on the top of the stack and
						// insert it
						// either
					...
					case Const.POP: {
						...
						// Pops either the top 2 category 1 values or a single
						// category 2
						// value
					...
					case Const.POP2: {
						...
						// Swaps the two category 1 types on the top of the
						// stack.
					case Const.SWAP: {
						break;
					}
					...
					case Const.LSTORE_3: {
						...
						// Push a value from the runtime constant pool. We'll
						// get these
					...
					case Const.ARRAYLENGTH: {
						...
						// Push small constants (-1..5) on the stack. These
						// literals are
					...
					// Opcodes that don't need any modifications. Here for
					// reference
