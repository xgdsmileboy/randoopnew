...
public final class Util {
	...
	public static String hangingParagraph(String string, int colWidth, int indentWidth) {
		...
		while (i.hasMoreWords()) {
			...
			if (line.length() == 0) {
				// If this happens, it will happen on all subsequence calls to
				// nextLine,
	...
	public static String createArgListJVML(Class<?>[] paramClasses) {
		...
		for (int i = 0; i < paramClasses.length; i++) {
			...
			// If primitive, Class.getName() returns the keyword. Convert to
			// JVML.
			if (cls.isPrimitive()) {
			...
			// representation,
			// except for the element class, which uses "." instead of "/" to
			// separate
			// package names.
			if (isArray) {
			...
			// Is object, non-array. Class.getName() returns foo.bar.Baz.
			// Convert to
			// JVML.
