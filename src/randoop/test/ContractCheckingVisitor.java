...
public final class ContractCheckingVisitor implements TestCheckGenerator {
	 ...
	 *
	 */
	public ContractCheckingVisitor(Set<ObjectContract> contracts, ExceptionPredicate exceptionPredicate) {
	...
	/**
	 * {@inheritDoc} Adds checks to final statement of sequence. Looks for
	 * failure exceptions, and violations of contracts in {@code contracts}.
	 */
	@Override
	public TestChecks visit(ExecutableSequence s) {
		...
		if (finalResult instanceof ExceptionalExecution) {
			// If there is an exception, check whether it is considered a
			// failure
		...
		for (Integer i : values) {
			for (Integer j : values) {
				...
				ExecutionOutcome exprOutcome = ObjectContractUtils.execute(c,
						((NormalExecution) result1).getRuntimeValue(), ((NormalExecution) result2).getRuntimeValue());
				...
				if (exprOutcome instanceof NormalExecution) {
					...
					// Execution of contract resulted in exception. Do not
					// create
					// a contract-violation decoration.
					// TODO are there cases where exception in contract check is
					// a
					// failure?
				} else {
	...
	private static MultiMap<ConcreteType, Integer> indicesToCheck(ExecutableSequence s) {
		...
		for (int i = 0; i < s.sequence.size(); i++) {
			...
			if (result instanceof NormalExecution) {
				...
				if (!outputType.equals(ConcreteTypes.VOID_TYPE) && !outputType.equals(ConcreteTypes.STRING_TYPE)
						&& !outputType.isPrimitive() && ((NormalExecution) result).getRuntimeValue() != null) {
