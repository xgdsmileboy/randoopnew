...
public final class RegressionCaptureVisitor implements TestCheckGenerator {
	...
	public RegressionCaptureVisitor(ExpectedExceptionCheckGen exceptionExpectation,
			MultiMap<ConcreteType, ConcreteOperation> observerMap, Set<ConcreteOperation> excludeSet,
	 ...
	 * try-catch block is always generated for exceptions, but whether
	 * assertions are included is determined by the
	 * {@link ExpectedExceptionCheckGen} given when creating this visitor.
	 *
	 * @throws Error
	 *             if any statement is not executed, or exception occurs before
	 *             last statement
	 */
	@Override
				...
				if (includeAssertions) {
					...
					if (o == null) {
						...
						if (o instanceof String) {
							// System.out.printf ("considering String check for
							// seq %08X\n",
							// s.seq_id());
							String str = (String) o;
							// Don't create assertions over strings that look
							// like raw object
							// references.
							if (PrimitiveTypes.looksLikeObjectToString(str)) {
								// System.out.printf ("ignoring Object.toString
								// obs %s%n", str);
								continue;
							}
							// Don't create assertions over strings that are
							// really
							// long, as this can cause the generate unit tests
							// to be
							// unreadable and/or non-compilable due to Java
							// restrictions on String constants.
							if (!PrimitiveTypes.stringLengthOK(str)) {
								if (Log.isLoggingOn()) {
									Log.logLine("Ignoring a string that exceeds the maximum length of "
						...
						// don't use it as it's just going to have today's date
						// in it.
						if (s.sequence.getInputs(i).size() > 0) {
							...
							if (var0.getType().hasRuntimeClass(java.util.Date.class)) {
								...
								if ((sk.isConstructorCall()) && (s.sequence.getInputs(i).size() == 1))
									continue;
								// System.out.printf ("var type %s comes from
								// date %s / %s%n",
						...
						checks.add(oc);
						// System.out.printf ("Adding objectcheck %s to seq
						// %08X\n",
						// oc, s.seq_id());
						...
						// Exception: if the value comes directly from a
						// constructor call,
						...
						if (observers != null) {
							for (ConcreteOperation m : observers) {
								...
								if (outcome instanceof ExceptionalExecution) {
									String msg = "unexpected error invoking observer " + m + " on " + var + "["
											+ var.getType() + "]" + " with value " + o + " [" + o.getClass() + "]";
								...
								// Don't create assertions over string that look
								// like raw object
								// references.
